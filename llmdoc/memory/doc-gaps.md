...
## G1. README "从 GitHub 拉取安装" 路径目前不可用 — 高 — **部分缓解 (2026-04-30)**

- **证据**：`git log` 显示 `main` 分支无 commit；`README.md` 第 19–25 行的 `claude plugin marketplace add https://github.com/itMrBoy/glab_skills.git` 即使作者期望仓库公开，远端目前也没有内容可拉。
- **影响**：团队成员当前只能用本地路径安装（`marketplace add C:\code\glab_skills`），README 第 20 行指引的 GitHub 安装路径会失败或拿到空仓库。
- **当前状态**：README 标题下已加 ⚠️ 提示，引导用户先用本地路径并指向本条 gap。
- **剩余补救**：完成首次 commit + push 后必须移除 README 顶部 ⚠️ 段并把本条 gap 改为"已解决"。详见 `guides/release-and-distribute.md` 场景 B。

## G2. README "完全跨工具兼容" 与 SKILL.md 内 Claude Code 专属工具表述冲突 — 中

- **证据**：`README.md` 第 30 行声明"完全跨工具兼容"。但 `skills/mr-diff/SKILL.md`、`skills/review-mr/SKILL.md`、`skills/review-fixup/SKILL.md` 正文多处直接以中文要求模型"用 Read 工具读取"、`Read llmdoc/index.md`、`Glob llmdoc/must/*.md`——这些是 Claude Code 内置 PascalCase 工具名，Codex CLI 工具空间不同。
- **影响**：Codex CLI 下严格意义上不能直接复用，得靠 LLM 把"用 Read 工具"翻译成等价文件读取（鲁棒性兜底而非协议兼容）。
- **建议补救**：
  - 把 README 表述改成"语义可移植，工具名需 Codex agent 自行映射"。
  - 或在 SKILL.md 里改用工具中性表达（"读取 `<path>`"代替"用 Read 工具读"）。
...
## G7. 缺 `reference/coding-conventions.md`（自食狗粮 gap）— 中 — **已解决 (2026-04-30)**
